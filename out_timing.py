...
create_table_sql = f"CREATE TABLE IF NOT EXISTS {table_name} (name TEXT, in_time TEXT, out_time TEXT, date DATE, UNIQUE(name, date))"
...
# Create an Excel workbook to store attendance data
wb = openpyxl.Workbook()
ws = wb.active
ws.append(["Name", "In Time", "Out Time", "Date"])
...
class Face_Recognizer:
    ...
    def attendance(self, name, in_time=None, out_time=None):
        ...
        # Find the row corresponding to the person for the current date
        mask = (df["Name"] == name) & (df["Date"] == current_date)
        person_record = df[mask]

        if person_record.empty:
            print(f"No attendance record found for {name} on {current_date}")
            return

        if pd.notnull(person_record["LeftTime"]).any():
            print(f"{name} already has a left time recorded for {current_date}")
            return

        # Update the "LeftTime" field with the current time
        df.loc[mask, "LeftTime"] = current_time
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                if self.current_frame_face_name_list[k] == 'unknown':
                                    ...
                                    self.attendance(nam, in_time=in_time)
                                else:
                                    # Mark out time
                                    out_time = datetime.datetime.now().strftime('%H:%M:%S')
                                    self.attendance(nam, out_time=out_time)
                            else:
