...
class Face_Recognizer:
    ...
    def attendance(self, name):
     ...
     in_time = datetime.datetime.strptime("09:15:00", "%H:%M:%S")  # Define the in-time

    # Check if the Excel file exists
     excel_file = "attendance.xlsx"
     if not os.path.exists(excel_file):
        # If the file doesn't exist, create a new DataFrame
        df = pd.DataFrame(columns=["Name", "InTime", "LateMark", "LeftTime", "Date"])
     else:
        # If the file exists, load the DataFrame from the Excel file
    ...
    # Check if the name already has an entry for the current date
     ...
     if not existing_entry.empty:
        ...
        # Calculate if the attendance is late
        attendance_time = datetime.datetime.strptime(current_time, "%H:%M:%S")
        late_mark = "On Time" if attendance_time <= in_time else "Late"

        # Insert a new row with the name, in-time, late-mark, left-time, and current date
        ...
        df.to_excel(excel_file, index=False)  # Save the DataFrame back to the Excel file

        if late_mark == "Late":
            print(f"{name} marked as present for {current_date} at {current_time} (Late)")
            # Update the count of late marks for this person in the month
            month = datetime.datetime.now().strftime('%Y-%m')
            late_marks_count = df[(df["Name"] == name) & (df["Date"].str.startswith(month)) & (df["LateMark"] == "Late")].shape[0]
            print(f"{name} has {late_marks_count} late marks in {month}")
        else:
